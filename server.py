...
from sheepbot26 import token, get_uptime_sec, get_uptime
import time
import datetime
import socket
...
class Server(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        #self.wfile.write(bytes(f"<script>var time = {get_uptime_sec()} " + "; var minutes = 0; var hours = 0; var intervalId = window.setInterval(function(){update_uptime()}, 1000); function update_uptime(){time += 1; if(time > 59){minutes += Math.round(time / 60); time = 0; if(minutes > 59){hours += Math.round(minutes / 24); minutes = 0;}} var uptime = document.getElementById('uptime'); uptime.innerHTML = ('Uptime: ' + hours + ':' + minutes + ':' + time)} function toggle_token() {var token = document.getElementById('token'); if(document.getElementById('token_button').innerHTML == 'Hide token'){ document.getElementById('token_button').innerHTML = 'Show token';} else{document.getElementById('token_button').innerHTML = 'Hide token'} if (token.style.display === 'none') {token.style.display = 'block';} else {token.style.display = 'none';}}</script>", "utf-8"))
